...
fn compile() -> Vec<String> {
    ...
    if !efl_lib.exists() || !efl_include.exists() {
        ...
        let installers = [
            ...
            ("https://download.enlightenment.org/rel/win/efl/efl-x86_64-win10-1.26.exe", "efl"),
            // Then try EWPI (EFL Windows Package Installer)
            ("https://download.enlightenment.org/rel/win/efl/ewpi-x86_64-win10-1.1.exe", "ewpi"),
        ];
            ...
            if !install_dir.exists() {
                ...
                execute_command(
                    Command::new(&installer_path)
                        .args(["/NCRC", "/S", &format!("/D={}", install_dir.display())])
                );
        ...
        if !efl_lib.exists() || !efl_include.exists() {
            ...
            eprintln!("You can also manually install EFL from: https://www.enlightenment.org/download");
            std::process::exit(1);
        }
    }
